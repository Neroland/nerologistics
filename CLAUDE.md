# Project context for AI coding agents — nerologistics
...
- Loader entry points: `NeroLogisticsFabric` (+ `NeroLogisticsFabricClient`), `NeroLogisticsForge`,
  `NeroLogisticsNeoForge` — each calls `NeroLogisticsCommon.init()` during construction.
- NeoForge/Forge debug tasks use `-PnerologisticsDebug`; Fabric Loom honours Gradle `--debug-jvm`.

## IDE (VS Code) run & debug

- Workspace: **`nerologistics.code-workspace`** (single-root `"."`). Import the Stonecutter nodes as **static
  ...
  VS Code. Per-node Eclipse project names are `nerologistics-<loader>-<mc>`.
...
- This mod has its own **dedicated wiki** in `wiki/` at the repo root: the player- and
  contributor-facing docs for NeroLogistics (features, blocks/items, machines, progression, recipes, FAQ).
...
- The wiki is **per-mod** — document only NeroLogistics here; cross-mod / ecosystem concepts live in the
