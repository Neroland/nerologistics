 ...
 * ships one {@link Factory} implementation, resolved via Core's {@link Services}
 * ({@link java.util.ServiceLoader}):
 * <ul>
 *   <li>NeoForge / Forge wrap a {@code DeferredRegister} (attached to NeroLogistics'
 *       mod bus by the loader entry point);</li>
 *   <li>Fabric calls {@code Registry.register} eagerly.</li>
 * </ul>
 *
 * <p>NeroLogistics ships its own copy of this seam (mirroring Nerotech) rather than
 * reusing {@code za.co.neroland.nerolandcore.registry.RegistrationProvider}: Core's
 * factory flushes its registers to Core's mod bus during Core construction, before
 * any downstream {@code init()} runs.
 */
