...
/**
 * The single door every NeroLogistics GUI goes through — {@code openMenu}, but it cannot take the
 * server thread with it when the host platform misbehaves (ported from Nerospace's {@code MenuOpener}).
 *
...
public final class MenuOpener {
    ...
    private static void recover(Player player, MenuProvider provider, Throwable cause) {
        ...
        NeroLogisticsCommon.LOGGER.warn(
                "[NeroLogistics] The server platform refused to open menu '{}' — the interaction was "
                        ...
                        + "supported platform for NeroLogistics.",
                menu, cause);
        ...
        try {
            player.sendSystemMessage(Component.translatable("gui.nerologistics.menu_open_failed"));
        } catch (RuntimeException | LinkageError ignored) {
            // The player simply does not get the notice.
        }
        NeroLogisticsTelemetry.captureHandledException(cause);
    }
